...
def train_model(df, features):
    ...
    def classify_threat(row):
        if row["predicted_anomaly"] == 0:
            return "Normal"
        if row["login_attempts"] > 10 and row["unusual_port"] and row["login_attempts"] > 15:
            return "Port Scan"
...
if __name__ == "__main__":
    df, features = load_and_preprocess("network_logs.csv")
    df, model, scaler, features = train_model(df, features)
    evaluate(df)
    df.to_csv("analyzed_logs.csv", index=False)
    joblib.dump({"model": model, "scaler": scaler, "features": features}, "model.pkl")
